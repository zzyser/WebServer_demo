...
void WebServer::InitEventMode_(int trigMode){
    listenEvent_=EPOLLRDHUP;                //检测到socket关闭
...
//处理监听socket，主要是accept新的socket，并加入timer和epoller中
...
void WebServer::DealRead_(HttpConn* client){
    assert(client);
    ExtentTime_(client);
    threadpool_->AddTask(bind(&WebServer::OnRead_,this,client));            //这是右值，bind将参数和函数绑定
}

//处理写事件，主逻辑是将OnWrite加入线程池的任务队列中
...
void WebServer::OnProcess(HttpConn* client){
    //首先调用process()进行逻辑处理
    if(client->process()){                  //根据返回的信息重新将fd置为EPOLLOUT（写） 或 EPOLLIN（读）
        //读完事件就跟内核说可以写了
